...
def cyclic_set_results(all_box_results):
    ...
    avg_res['info']['simname'] = f"AVG_{firstbox}_{lastphase}"
...
if __name__ == "__main__":
    ...
    for dataset in ["ELG_rd", "LRG_rd"]: 
        ...
        for hod_recon_model in hod_recon_model_dirs[0]: # if we passed the asserts above, we can just use one box's list of HOD models. 
            ...
            cov_pre_rec_xi02  = mmread(f"xi_cov_analytic_{REDSHIFT}_ph000_pre_recon_e-8_shot.mtx")
            cov_post_rec_xi02 = mmread(f"xi_cov_analytic_{REDSHIFT}_ph000_post_recon_e-8_shot.mtx")
